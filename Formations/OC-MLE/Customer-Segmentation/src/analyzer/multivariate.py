from src.analyzer.univariate import np, plt, sns, Subplot
...
# Normal distribution subplots

def plot_norm_subplots(df, cols, shape, fig_size=(14, 18), x_pad=5, y_pad=5, title_pad=15, h=.65, w=.5):
    subplot = Subplot(shape, fig_size, cols)
    data = lambda col: st.boxcox(df[col].values)[0]
    probplot_func = lambda a, col: st.probplot(data(col), dist=st.norm, plot=a)
    subplot.plot_data(plot_function=probplot_func, df=None, title=None, title_pad=title_pad, x_pad=x_pad, y_pad=y_pad)
    subplot.fig.subplots_adjust(hspace=h, wspace=w)
    return

# Old

# def plot_norm_subplots(df, cols, n_rows, n_cols, fig_size=(14, 18), x_pad=5, y_pad=5, title_pad=15, h=.65, w=.5):
...
#         xt, _ = st.boxcox(df[col].values)
#         prob = st.probplot(xt, dist=st.norm, plot=a)
