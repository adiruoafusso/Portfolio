...
def filter_corrupted_images(filepaths, valid_header="JFIF", verbose=True):
    """
    :param filepaths: dictionary which contains classes as keys and absolute image file paths as values
    :param valid_header:
    :param verbose:
    """
    # Filter out corrupted images
    skipped_images_ct = 0
    for folder_name, image_paths in filepaths.items():
        for image_path in image_paths:
            try:
                fobj = open(image_path, "rb")
                is_jfif = tf.compat.as_bytes(valid_header) in fobj.peek(10)
            finally:
                fobj.close()
                if not is_jfif:
                    if verbose:
                    ...
                    skipped_images_ct += 1
    print(f"Deleted {skipped_images_ct} images")
    return
