...
def plot_n_top_values(df,
    ...
    for p in ax.patches:
        percentage = '{} ({:.{}f}%)'.format(p.get_width(), 100 * p.get_width() / total, precision)
...
def count_plot(df,
    ...
    if bar_type is 'v':
        ax = sns.countplot(x=variable, data=df[:n])
        get_percentage_numerator = lambda p: p.get_height()
    elif bar_type is 'h':
        ax = sns.countplot(y=variable, data=df[:n])
        get_percentage_numerator = lambda p: p.get_width()
    ylabel = variable.replace('_', ' ') if ylabel is None else ylabel
    plt.title('Distribution of {}'.format(ylabel) if title is None else title, pad=20)
    plt.xlabel(xlabel, labelpad=20)
    plt.ylabel(ylabel, labelpad=20)
    total = len(df[variable])
    for p in ax.patches:
        percentage = '{:.{}f}%'.format(100 * get_percentage_numerator(p) / total, precision)
