...
def get_confusion_matrix(y_labels, logits, class_names):
    ...
    cm = confusion_matrix(y_labels, preds, labels=np.arange(len(class_names)))
    return cm
...
# Tensorflow version
def plot_training_curve(history, value_type='accuracy', fine_tuning=False, initial_epochs=None):
    """
